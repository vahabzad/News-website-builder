...
export default function ProjectPage() {
  ...
  return <main className="container"><nav className="topbar"><Link href="/" className="back">→ داشبورد</Link><span className={`status-tag ${project.status}`}>{statusLabel(project.status)}</span></nav><section className="project-detail glass"><div className="detail-head"><div><span className="eyebrow">LOCAL CODEX PROJECT / V{project.version}</span><h1>{project.name}</h1><p>آخرین تغییر: {new Date(project.updatedAt).toLocaleString("fa-IR")}</p><code className="local-path">{project.localPath}</code></div>{working && <LiveBuildPanel progress={progress} status={project.status} updatedAt={project.updatedAt} now={now} />}</div>{project.error && <div className="error-box"><b>ساخت ناموفق بود</b><pre>{project.error}</pre><button className="secondary" onClick={retry} disabled={sending}>{sending ? "در حال ارسال..." : "تلاش مجدد"}</button></div>}{project.status === "ready" && <><div className="result-actions"><Link className="primary button-link" href={`/preview/${id}`} target="_blank">مشاهده پیش‌نمایش</Link><button className="secondary" onClick={download}>دانلود ZIP</button></div><div className="preview-frame"><iframe title={`پیش‌نمایش ${project.name}`} src={`${API_URL.replace(/\/api$/, "")}/preview/${id}/`} /></div></>}{project.continuationEnabled && project.status === "ready" && <form className="continue-box" onSubmit={continueProject}><h2>ادامه و ویرایش پروژه</h2><p>تغییر موردنظر را بنویسید؛ همان Thread و پروژه محلی Codex ادامه پیدا می‌کند.</p><textarea name="instruction" required minLength={3} maxLength={4000} placeholder="مثلاً هدر را تیره کن و بخش ویدئو را بالاتر بیاور..." /><button className="primary" disabled={sending}>{sending ? "در حال ارسال..." : "اعمال تغییرات"}</button></form>}{!project.continuationEnabled && <p className="notice">امکان ادامه دادن این پروژه فعلاً غیرفعال است.</p>}{error && <p className="error">{error}</p>}</section></main>;
}
